...
test('create member usecase success', async () => {
  ...
  const member = await usecase.call(
    "9b2f4e8c-8d59-11eb-8dcd-0242ac130003", "Adam Leviners", "Brownas123!"
  )
...
test('create member usecase event not found', async () => {
  ...
  try{
    await usecase.call(
      "9b2f4e8c-8d59-11eb-8dcd-0242ac130004", "Adam Leviners", "Brownas123!"
    )
  }
  catch(err: any){
    expect(err.message).toBe('Event not found for eventId: 9b2f4e8c-8d59-11eb-8dcd-0242ac130004')
  }
  repo.resetMock()
})
...
test('create member usecase member duplicated', async () => {
  ...
  try{
    await usecase.call(
      "9b2f4e8c-8d59-11eb-8dcd-0242ac130003", "Adam Levine", "Brownas123!"
    )
  }
  catch(err: any){
